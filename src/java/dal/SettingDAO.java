...
public class SettingDAO extends DBContext {
    ...
    public List<Setting> getAll() {
        ...
        try {
            String strSelect = "SELECT s.id,type_id,value,s.name,`order`,"
                    ...
                    + "on s.type_id = t.id order by s.id";
            stm = connection.prepareStatement(strSelect);
            rs = stm.executeQuery();
            while (rs.next()) {
                Setting setting = new Setting(rs.getInt("id"),
                        rs.getInt("type_Id"), rs.getInt("order"), rs.getString("name"), rs.getString("type"),
    ...
    public List<Setting> getAllByName(String n) {
        ...
        try {
            String strSelect = "SELECT s.id,type_id,value,s.name,`order`,"
                    + " status, description,t.name as type FROM setting as s join setting_type as t "
                    + "on s.type_id = t.id where s.name like ?";
            stm = connection.prepareStatement(strSelect);
            stm.setString(1, "%" + n + "%");
            rs = stm.executeQuery();
            while (rs.next()) {
    ...
    public static void main(String[] args) {
        ...
        Setting st = new Setting(1, 2, 1, "SẢn Phẩm cho mèo", "Active", "Pro Cate", "hehehe");
