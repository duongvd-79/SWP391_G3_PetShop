...
public class FeedbackDetailServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            if (user != null) {
                ProductFeedback f = fdao.getFeedbackByID(feedbackID);
                ArrayList<ProductFeedback> fmlist = fdao.getFeedbackImage(feedbackID);
                Product p = pdao.getProductById(f.getProductId());
                Address a = adao.getAddressByUserId(user.getId());
                
                request.setAttribute("address", a);
                request.setAttribute("feedback", f);
                request.setAttribute("imageList", fmlist);
            ...
            Logger.getLogger(FeedbackDetailServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    } 
